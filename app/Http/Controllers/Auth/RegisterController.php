...
class RegisterController extends Controller
{
    ...
    */
    ...
    protected function validator(array $data)
    {
        ...
        $rules = [
            'country_code' => ($registerMethod == 'email') ? 'required' : 'nullable',
            'mobile' => (($registerMethod == 'email') ? 'required' : 'nullable') . '|numeric|unique:users',
    ...
    public function register(Request $request)
    {
        ...
        if ($user->role_name == 'user') {
            ...
            WebhookService::send('user_created', [
            'user_email' => $user->email,
            'user_name'  => $user->name ?? ($user->full_name ?? null),
            'user_id'    => $user->id,
            'mobile'    =>  $user->mobile ?? "00000000",
            'date'       => "08/25",
         ...
         WebhookService::send('trainer_created', [
            ...
            'mobile'    =>  $user->mobile ?? "00000000",
            'date'       => "08/25",
            "tags"        => "Trainers Emails",
            ]);

        }elseif($user->role_name == 'organization') {


            WebhookService::send('tp_domain_created', [
            'user_email' => $user->email,
            'user_name'  => $user->name ?? ($user->full_name ?? null),
            'user_id'    => $user->id,
            'mobile'    =>  $user->mobile ?? "00000000",
            'date'       => "08/25",
            "tags"        => "Organizations Emails",
            ]);
